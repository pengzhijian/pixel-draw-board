 ...
 */
function drawRulers(canvas: HTMLCanvasElement, setting: RulerSetting = { unitLength: 10, majorTickLength: 10, minorTickLength: 5, globalAlpha: 1, translateX: 0, translateY: 0 }) {
  const ctx: CanvasRenderingContext2D = canvas.getContext("2d") as CanvasRenderingContext2D;
  const { unitLength, majorTickLength, minorTickLength, globalAlpha, translateX, translateY } = setting;
  // 设置透明度
 ...
 */
function drawContent(canvas: HTMLCanvasElement, canvasSetting: CanvasSetting = { translateX: 0, translateY: 0, scaleX: 1, scaleY: 1 }) {
  const ctx: CanvasRenderingContext2D = canvas.getContext("2d") as CanvasRenderingContext2D;
...
function drawAll() {
  if (canvas.value) {
    drawContent(canvas.value, { translateX: baseSetting.translateX, translateY: baseSetting.translateY, scaleX: baseSetting.scaleX, scaleY: baseSetting.scaleY });
    drawRulers(canvas.value, { translateX: baseSetting.translateX, translateY: baseSetting.translateY, unitLength: 10, majorTickLength: 10, minorTickLength: 5, globalAlpha: 1 });
  }
}
